...
public class bplustree implements Serializable {
	...
	LeafNode firstLeaf;
//	min w el max
//	search upperbound key retrun search(min,key)
	Comparable min;
	Comparable max;
...
//			if (key < keys[i]) { break; }
			if (key.compareTo(keys[i])<=-1) { break; }
		}
...
//					values.add(dp.value);
//				}
				int lower = (lowerBoundInclusive? 0: -1);
				int upper = (upperBoundInclusive? 0: -1);
				if(lowerBound.compareTo(dp.key)<=lower && dp.key.compareTo(upperBound)<=upper){
					values.add(dp.value);
				}
			}
...
//			boolean firstLine = true;
			...
			bpt.delete("0102");
			bpt.delete("0021");
			bpt.insert("0106","-3.91");
			bpt.insert("0023","3.55");
//			ArrayList<HashSet<String>> temp2 = bpt.search("0023","0099");
